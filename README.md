...
* `Ts2Tp` 将无间隔的 `.tscps` 原稿转换为带逐字时间的 `.tscp`，支持逐句和连续设计。
  ...
  清屏都会执行），播放中按 Esc 停止。
* `PlotManager` 是剧情素材管理器：新建 `.tscpkg`、插入音乐（自动写入 `CONFIG`
  简称）、导入 `.tscp` 或把 `.tscps` 编译后导入、直接在包内编辑剧本、修改剧情
  名称与简介。导出有两条路：**「导出副本 .tscpkg」**把当前包另存一份，
  **「文件夹导出为 .tscpkg」**把带 `Musics` + `Scripts` 的整个文件夹打包成单
  文件。编辑时没有单独的保存步骤，插入即写入当前包。
